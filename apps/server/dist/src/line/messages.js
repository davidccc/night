...
export function buildCustomerServiceMessage() {
    const lines = [
        '👩‍💼 已轉接至真人客服，請稍候。',
        '若客服忙碌，可先留言你的需求與聯絡方式。',
    ];
...
export function buildSweetCarousel(sweets) {
    const contents = sweets.map((sweet) => ({
        type: 'bubble',
        hero: {
            type: 'image',
...
function buildBodyContents(name, description, tag) {
    const contents = [
        {
            type: 'text',
            text: name,
